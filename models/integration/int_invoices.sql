...
with t_invoices_merge_list as (
  ...
  {% for source in var('finance_warehouse_invoice_sources') %}
    ...
    all_invoice_ids as (
       ...
       max(expected_payment_at_ts) as expected_payment_at_ts,
       max(invoice_period_start_at_ts) as invoice_period_start_at_ts,
       max(invoice_period_end_at_ts) as invoice_period_end_at_ts,
       sum(invoice_local_total_revenue_amount) as invoice_local_total_revenue_amount,
       max(invoice_currency) as invoice_currency,
       max(invoice_currency_rate) as invoice_currency_rate,
       max(total_local_amount) as total_local_amount,
       max(total_gbp_amount) as total_gbp_amount,
       {% if 'harvest_projects' in var("projects_warehouse_timesheet_sources") %}
       sum(invoice_local_total_billed_amount) as invoice_local_total_billed_amount,
       sum(invoice_local_total_services_amount) as invoice_local_total_services_amount,
       sum(invoice_local_total_licence_referral_fee_amount) as invoice_local_total_licence_referral_fee_amount,
       sum(invoice_local_total_expenses_amount) as invoice_local_total_expenses_amount,
       sum(invoice_local_total_support_amount) as invoice_local_total_support_amount,
       {% endif %}
       max(invoice_tax_rate_pct) as invoice_tax_rate_pct,
       sum(invoice_local_total_tax_amount) as invoice_local_total_tax_amount,
       sum(invoice_local_total_due_amount) as invoice_local_total_due_amount,
       ...
       group by 1),
    joined as (
      SELECT i.* except (invoice_status),
      case when invoice_status = 'Open' and invoice_due_at_ts<current_timestamp then 'Overdue' else invoice_status end as invoice_status,
      a.all_invoice_ids,
