...
renamed as (
  SELECT
    concat('hubspot-',engagement_id)                    as conversation_id,
    concat('hubspot-',contactids.value)                  as conversation_user_id,
    concat('hubspot-',contactids.value)                  as conversation_author_id,
    ...
    concat('hubspot-',contactids.value)  AS conversation_assignee_id,
    cast (null as string)    AS conversation_assignee_state,
    concat('hubspot-',engagement_id )  AS conversation_message_id,
    ...
    coalesce(metadata.from.email,cast(null as string))    as  conversation_from_email,
    coalesce(concat(metadata.from.firstname,' ',metadata.from.lastname),cast(null as string))    as  conversation_from_contact_name,
    coalesce(metadata_to.value.email,cast(null as string))    as  conversation_to_email,
