...
select i.*,
  ...
  {{ dbt_utils.datediff('date(first_value(i.sent_at) over (partition by i.client_id order by i.sent_at))','date(i.sent_at)','MONTH') }} as months_since_first_invoice,
  {{ dbt_utils.datediff('date(first_value(i.sent_at) over (partition by i.client_id order by i.sent_at))','date(i.sent_at)','QUARTER') }} as quarters_since_first_invoice,
...
renamed as (
select  cast(number as string) as invoice_number,
        ...
        cast(null as timestamp) as expected_payment_at_ts,
        ...
        cast(null as float64) as invoice_currency_rate,
        amount as total_local_amount,
        cast(null as float64) as total_gbp_amount,
        ...
        tax_amount as invoice_local_total_tax_amount,
...
where number not in (
  ...
  from
  {{ ref('deleted_timesheet_invoices')}}
)
)
SELECT
  *
