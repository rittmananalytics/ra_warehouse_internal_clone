...
select concat('{{ var('stg_jira_projects_id-prefix') }}',id) as task_id,
       ...
       cast(fields.customfield_10026 as float64)	as task_story_points,
       case when fields.status.name	 = 'To Do' then 1
            ...
            when fields.status.name  = 'Failed Internal QA' then 8
            when fields.status.name	 = 'In Client QA' then 9
            when fields.status.name  = 'Failed Client QA/QA Comment' then 10
            when fields.status.name  = 'Revision Required' then 11
            when fields.status.name	 in ('Done','Done/Passed Client QA') then 12
      ...
      case when fields.status.name	 not in ('Done','Done/Passed Client QA') then timestamp_diff({{ dbt_utils.current_timestamp() }},fields.created,HOUR)
       ...
       case when fields.status.name = 'Revision Required' then 1 end as total_revision_required,
       case when fields.status.name = 'Failed Internal QA' then 1 end as total_failed_internal_qa,
       1 as total_issues,
       max(timestamp(customfield_10018.value.startdate)) over (partition by id order by timestamp(customfield_10018.value.startdate) RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as last_task_start_ts,
...
flagged as (
 select
  *,
  case when last_task_start_ts = task_start_ts then true else false end as is_latest_task_version
from renamed
)
SELECT
 *
