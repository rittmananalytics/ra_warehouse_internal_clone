...
{% if 'linkedin_likers' in var("crm_warehouse_contact_sources") %}
...
WITH contacts AS (
  ...
  (
    SELECT
      ...
      lastName,
      job,
      first_value(`timestamp`) over (partition by name order by `timestamp`) as created_ts,
      Row_Updated_At
    FROM {{ source('linkedin_likers','contacts' ) }}
  )
...
SELECT
  ...
  cast(coalesce(p.jobTitle,c.job) as string) contact_job_title,
  CAST(p.email AS string) contact_email,
  CAST(NULL AS string) contact_phone,
  coalesce(p.location,CAST(NULL AS string)) contact_address,
  ...
  c.created_ts  contact_created_date,
