...
SELECT
   ...
   {{ dbt_utils.date_trunc('MONTH',
                           'MIN(date(invoice_sent_at_ts))
                              OVER (partition by c.company_pk
   ...
   {{ dbt_utils.datediff('invoice_sent_at_ts','max(date(invoice_sent_at_ts)) over (partition by c.company_pk RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)', 'MONTH') }}  as months_before_last_invoice,
...
LEFT OUTER JOIN projects_dim p
   ON cast(i.project_id as string) = cast(p.timesheet_project_id as string)
{% endif %}
